...
{
	if (SIM808_SendString ("AT\r") == SIM_ER)
	{
		while (1)
			; // assert
	}
	...
	HAL_Delay (1000);
	if (HAL_UART_Receive_IT ( & SIM808_UART, (uint8_t *)& SIM808_rx_byte, 1) != HAL_OK)
	{
		while (1)
			; // assert
	}
...
{
	if (HAL_UART_Transmit_DMA ( & SIM808_UART, (uint8_t *) cmd, strlen (cmd)) == HAL_OK)
		return SIM_OK;
	else
		return SIM_ER;
}
...
SIM808Status SIM808_Dial (uint32_t timeout_ms)
{
	...
	SIM808_DelayUntilAlarmFlag0 (timeout_ms);
...
SIM808Status SIM808_SendSMS (uint32_t timeout_ms, char* msg)
{
	...
	if (SIM808_SendATCommand ("AT+CMGF=1\r", "\r\nOK\r\n", NULL, timeout_ms) == SIM_OK)
	{
		if (SIM808_SendATCommand (tempStr, "\r\n> ", NULL, timeout_ms) == SIM_OK)
		{
			...
			strlcat (tempStr, "\x1A", sizeof (tempStr)); // delimiter for the end of the message
			SIM808_SendString (tempStr);
			SIM808_DelayUntilAlarmFlag0 (1000);
			SIM808_SendString ("AT+CMGF=0\r");
			SIM808_DelayUntilAlarmFlag0 (100);
			return SIM_OK;
		}
	}
	return SIM_ER;
...
// not implemented as we have a problem with the signal
